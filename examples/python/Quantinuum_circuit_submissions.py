...
# When using `pytket` before submitting to hardware, the `get_compiled_circuit` function serves to (1) rebase circuits to the hardware's native gate set, (2) perform circuit optimizations, if desired. Because the TKET compilation passes have been integrated into the H-Series stack, performing circuit optimizations is redundant before submitting to hardware, unless the user would like to see the optimizations applied before submitting. 

# The `optimisation_level` parameter in the `get_compiled_circuit` function corresponds directly to the the level of optimisation after submitting to the H-Series systems and to the `tket-opt-level` parameter in the H-Series API. Note that the **default compilation pass for the `get_compiled_circuit` function is optimization level 1** instead of 2.

# Given that the TKET compilations passes are integrated into the Quantinuum systems software stack, users may take 1 of 3 approaches when submitting jobs:
...
# This section leverages the discussion in the [Circuit Compilation](#circuit-compilation) section to illsutrate how to turn TKET compilations on or off in the `process_circuit` function, specifically as for options 2 and 3.

# If found during exploration that it is desired to use TKET optimization levels 1, 0, or no optimizations at all, this can be done in the `process_circuit` function using the `tket-opt-level` parameter.

# For option 2, suppose a user explores the results of TKET compilation passes on a circuit and finds that `optimisation_level=1` is desirable, and would like this circuit to be the one that runs on H-Series systems. The submission below specifies this in `get_compiled_circuit` function with optimization level 1. Because the circuit is optimized beforehand, the TKET optimization in the H-Series stack should be turned off since and needs to be given since the default is optimization level 2. The value `tket-opt-level:None` turns off TKET optimization in the H-Series stack.
...
# For option 3, suppose a user wants to turn off all optimizations in the stack, even simple single-qubit combinations done by the H-Series compiler. This can be done by setting `optimisation_level=0` in `get_compiled_circuit` and setting `tket-opt-level:None` in the `process_circuits` function.
