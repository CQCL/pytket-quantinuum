...
.. note:: pytket-quantinuum is not compatible with Quantinuum Nexus. For guidance on how to access H-Series through Nexus, please see the `Nexus documentation <https://docs.quantinuum.com/nexus>`_ and the `qnexus <https://pypi.org/project/qnexus/>`_ Python package.

.. note::   Running circuits remotely on the ``QuantinuumBackend`` requires a `Quantinuum <https://www.quantinuum.com/>`_ account.
...
* ``H1-1LE``, a version of the ``H1-1E`` emulator that runs locally. For running simulations locally see the docs on `Local Emulators`_.
